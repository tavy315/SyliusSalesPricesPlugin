function recalculatePrices(quantity) {
    $('#sales_prices_tables').children().each(function (i, table) {
        var jQueryTable = $(table);
        if (jQueryTable.css('display') !== 'none') {

            var price = getPricesFromSalesPricesTable(table, quantity);
            if (price === null) {
...
function getPricesFromSalesPricesTable(element, quantity) {
